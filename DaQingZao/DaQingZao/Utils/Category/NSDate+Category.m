...
- (NSString *)formattedDateDescription
{
    ...
    if (timeInterval < 60) {
        ...
        NSString * str = [NSString stringWithFormat:@"%ld分钟前", (long)timeInterval / 60];
        return str;
...
-(NSString *)formattedDateDescriptionWithFutureTime
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    ...
    if(timeInterval <0)
    {
        ...
        return [NSString stringWithFormat:@"%ld分钟后", (long)timeInterval / 60];
    } else if (timeInterval < 86400) {
        return [NSString stringWithFormat:@"%ld小时后", (long)timeInterval / 3600];
    } else if (timeInterval < 2592000) {//30天内
        return [NSString stringWithFormat:@"%ld天后", (long)timeInterval / 86400];
    } else if (timeInterval < 31536000) {//30天至1年内
        return [NSString stringWithFormat:@"%ld月后",(long)timeInterval /2592000];
    } else {
        NSInteger timeTravel =timeInterval / 31536000;
        if(timeTravel>100)
        {
            return @"很久以后";
        }
        return [NSString stringWithFormat:@"%ld年后", (long)timeTravel];
    }}
